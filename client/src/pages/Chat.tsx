...
const Chat = () => {
  ...
  const [chatsLoaded, setChatsLoaded] = useState(false); // Prevent repeated API calls
  ...
  useEffect(() => {
    if (auth?.user && !chatsLoaded) {
      setChatsLoaded(true); // Ensure it runs only once
  ...
  const onSubmit = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (chatData?.chats) {
        setChatMessages((prev) => [...prev, chatData.chats.slice(-1)[0]]);
      }
    } catch (error) {
  ...
  return (
    <div className="h-screen w-screen bg-zinc-900 flex flex-col overflow-hidden">
      <Header>
        {/* User initials and delete button in header for small screens */}
      ...
      <div className="px-3 py-2 h-[88vh] gap-5 flex md:justify-between bg-zinc-900 w-screen">
        ...
        <div className="px-2 h-96 w-72 bg-zinc-800 flex-col justify-evenly rounded-lg items-center hidden md:flex">
          ...
          <button onClick={handleDeleteChats} className="py-2.5 px-5 text-sm font-medium text-white bg-red-600 rounded-full hover:bg-red-700">
            Clear Conversation
          </button>
        </div>
        <div className="flex-1 w-full md:w-[78.5vw] rounded-lg bg-zinc-800 flex flex-col relative z-20">
          <div className="flex-grow overflow-y-auto p-3 pb-3 relative z-30" ref={chatContainerRef}>
            {chatMessages.length > 0 ? (
              chatMessages.map((chat, index) => (
                <ChatItem content={chat.content} role={chat.role} key={index} />
              ))
            ) : (
              <p className="text-white text-center mt-48 text-xl">No chats yet. Start the conversation!</p>
            )}
            {loading && (
          ...
          <div className="p-3 relative z-40 flex items-center gap-3">
            <input type="text" ref={inputRef} placeholder="Type a message..." className="flex-grow p-2 rounded-lg bg-zinc-700 text-white focus:outline-none" onKeyDown={(e) => e.key === "Enter" && onSubmit(e)} disabled={loading} />
            <button onClick={onSubmit} className="px-4 py-2 bg-zinc-700 text-white rounded-lg flex items-center justify-center min-w-[80px]" disabled={loading}>
